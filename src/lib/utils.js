...
export function hoursToText(opening_hours) {
  if (!opening_hours) return '';
  if (typeof opening_hours === 'string') return opening_hours;
  return opening_hours.text ?? JSON.stringify(opening_hours, null, 2);
}

/** Lista unikalnych miast posortowana po polsku */
export function uniqueCities(items) {
  return [...new Set(items.map((i) => i.city).filter(Boolean))].sort((a, b) =>
